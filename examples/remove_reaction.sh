...
remove_reaction() {
    ...
    if [ -z "$reaction" ] || [ -z "$channel" ] || [ -z "$ts" ]; then
        echo "Usage: remove_reaction <reaction> <channel> <timestamp>"
        return 1
    fi
    
    local json_message=$(cat <<EOF
{"reaction":"$reaction","channel":"$channel","ts":"$ts","remove":true}
EOF
)
    
    docker exec slackliner-redis redis-cli RPUSH "$REDIS_REACTION_LIST_KEY" "$json_message"
    echo "Reaction removal request pushed to Redis queue"
}

# Example usage
if [ "$#" -eq 3 ]; then
    remove_reaction "$1" "$2" "$3"
else
    ...
    echo ""
    echo "Note: This removes a reaction from a message. The reaction must have been"
    echo "      previously added to the message. You can get the timestamp (ts) from"
    echo "      the message posting response or by inspecting the message in Slack's API."
    exit 1
fi
